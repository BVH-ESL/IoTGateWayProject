...
RF24 radio(4,2);                          // Set up nRF24L01 radio on SPI bus plus pins 7 & 8
...
//  printf_begin();
  ...
  attachInterrupt(5, check_radio, FALLING);             // Attach interrupt handler to interrupt #0 (using pin 2) on BOTH the sender and receiver
  interrupts();
}
