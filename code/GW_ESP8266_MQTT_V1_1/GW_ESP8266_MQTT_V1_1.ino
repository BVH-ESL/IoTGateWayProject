...
void handle_rx() {
  //  if (cmd == CMD_CHECK) {
  //    sprintf(oTopic, "esl/%s/0x%x/0x%x/out/status\0",
  //                      sensorNodeLink, sensorNodeAddr, sensorNodePAN);
  //  }
  //  else if (cmd == CMD_TEMP) {
  //    sprintf(oTopic, "esl/%s/%x/%x/out/data\0",
  //                      sensorNodeLink, sensorNodeAddr, sensorNodePAN);
  //  }
//char aTopic [64];/
  char * payloadTmp;//
  ...
  if (payloadTmp != NULL) {
    delay(10);
//    if(!strlen(oTopic)){/
//    sprintf(aTopic, "esl/%s/0x%x/0x%x/out/data",
//            sensorNodeLink, sensorNodeAddr, sensorNodePAN);
//    }/
    Serial.println(strlen(oTopic));
    Serial.println(oTopic);
    Serial.println("clear oTopic at RX2");
...
//  char oTopic[64];/
  ...
  strncpy(oTopic, payloadDummy, 63);
  oTopic[63] = '\0';
  Serial.println("clear oTopic at TX");
  if (cmd == CMD_CHECK) {
      sprintf(oTopic, "esl/%s/0x%x/0x%x/out/status\0",
                        sensorNodeLink, sensorNodeAddr, sensorNodePAN);
    }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  while (topicTmp != NULL) {
    if ( !strcmp(topicTmp, "mrf")) {
      link = LT_MRF;
      sensorNodeLink = topicTmp;
      sensorNodeLink[strlen(sensorNodeLink)] = '\0';
